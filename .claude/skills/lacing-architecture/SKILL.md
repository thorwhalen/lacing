...
| 8 | **PROV-O provenance inline on every annotation.** `was_generated_by`, `was_derived_from`, `was_attributed_to`, `generated_at_time`. AI annotations carry `agent:<model>@<hash>`. `generated_at_time` is wall-clock (`RationalTime.now()`); tick 0 is the `UNKNOWN_GENERATED_AT` sentinel, never the epoch — check `Provenance.generated_at_is_known` before ordering, and treat unknown as unverifiable (stale), not oldest (lacing#44). | ANN-DOC §C; BACK-DOC §4.5 |
...
- **Phase 2** — FastAPI server + Arq workers + MCP + OpenTelemetry. **Done.** REST CRUD + ETag-based optimistic concurrency + import/export + schema introspection + op-log + `/state-at` time-travel + MCP server (10 tools — agents as first-class clients) + processor registry (`lacing/processors.py`) with built-in `low_confidence_review` and `detect_density_change_points` + optional Arq integration (`lacing/worker.py`) + **OpenTelemetry instrumentation (`lacing/otel.py`) with no-op fallback and an opt-in `instrument_app` middleware that tags spans with the Lamport clock**.
