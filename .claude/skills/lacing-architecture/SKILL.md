...
- **Phase 2** — FastAPI server + Arq workers + MCP + OpenTelemetry. **Partially done:** REST CRUD + ETag-based optimistic concurrency + import/export + schema introspection + op-log + `/state-at` time-travel endpoint are in (`lacing/server/`, `lacing/oplog.py`). **Remaining:** Arq background workers, MCP server, OpenTelemetry instrumentation.
